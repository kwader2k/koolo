...
func equipBestItems(itemsByLoc map[item.LocationType][]data.Item, target item.LocationType) (bool, error) {
	...
	for loc, items := range itemsByLoc {
		...
		ctx.Logger.Info(fmt.Sprintf("Attempting to equip %s to %s", bestItem.Name, loc))
		err := equip(bestItem, loc, target)
		if err == nil {
			ctx.Logger.Info(fmt.Sprintf("Successfully equipped %s to %s", bestItem.Name, loc))
		...
		ctx.Logger.Error(fmt.Sprintf("Failed to equip %s to %s: %v", bestItem.Name, loc, err))
	}
...
func equip(itm data.Item, bodyloc item.LocationType, target item.LocationType) error {
		...
		if target == item.LocationMercenary {
			...
			if currentlyEquipped.UnitID != 0 {
				...
				if ctx.Data.LegacyGraphics {
					switch bodyloc {
					case item.LocHead:
						slotCoords = data.Position{X: ui.EquipHeadClassicX, Y: ui.EquipHeadClassicY}
					case item.LocNeck:
						slotCoords = data.Position{X: ui.EquipNeckClassicX, Y: ui.EquipNeckClassicY}
					case item.LocTorso:
						slotCoords = data.Position{X: ui.EquipTorsClassicX, Y: ui.EquipTorsClassicY}
					...
					case item.LocLeftRing:
						slotCoords = data.Position{X: ui.EquipLRinClassicX, Y: ui.EquipLRinClassicY}
					case item.LocRightRing:
						slotCoords = data.Position{X: ui.EquipRRinClassicX, Y: ui.EquipRRinClassicY}
					case item.LocBelt:
						slotCoords = data.Position{X: ui.EquipBeltClassicX, Y: ui.EquipBeltClassicY}
					case item.LocFeet:
						slotCoords = data.Position{X: ui.EquipFeetClassicX, Y: ui.EquipFeetClassicY}
					case item.LocGloves:
						slotCoords = data.Position{X: ui.EquipGlovClassicX, Y: ui.EquipGlovClassicY}
					}
				} else {
					switch bodyloc {
					case item.LocHead:
						slotCoords = data.Position{X: ui.EquipHeadX, Y: ui.EquipHeadY}
					case item.LocNeck:
						slotCoords = data.Position{X: ui.EquipNeckX, Y: ui.EquipNeckY}
					case item.LocTorso:
						slotCoords = data.Position{X: ui.EquipTorsX, Y: ui.EquipTorsY}
					case item.LocLeftArm:
						slotCoords = data.Position{X: ui.EquipLArmX, Y: ui.EquipLArmY}
					case item.LocRightArm:
						slotCoords = data.Position{X: ui.EquipRArmX, Y: ui.EquipRArmY}
					case item.LocLeftRing:
						slotCoords = data.Position{X: ui.EquipLRinX, Y: ui.EquipLRinY}
					case item.LocRightRing:
						slotCoords = data.Position{X: ui.EquipRRinX, Y: ui.EquipRRinY}
					case item.LocBelt:
						slotCoords = data.Position{X: ui.EquipBeltX, Y: ui.EquipBeltY}
					case item.LocFeet:
						slotCoords = data.Position{X: ui.EquipFeetX, Y: ui.EquipFeetY}
					case item.LocGloves:
						slotCoords = data.Position{X: ui.EquipGlovX, Y: ui.EquipGlovY}
					}
				}
				...
				if slotCoords.X > 0 {
					...
					for i := 0; i < 3; i++ {
						utils.Sleep(400)
						*ctx.Data = ctx.GameReader.GetData()
						if GetEquippedItem(ctx.Data.Inventory, bodyloc).UnitID == 0 {
							unequipped = true
							break
						}
					}
			...
			// Use Shift + Left Click to equip the new item instead of Right Click
	...
	return fmt.Errorf("verification failed after all attempts to equip %s", itm.Name)
}
