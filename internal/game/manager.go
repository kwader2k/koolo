...
func (gm *Manager) ExitGame() error {
	...
	// "Save and Exit Game" button Y positions to try (as percentage of screen height)
	// The quit menu in D2R has: Options, Save and Exit Game, Return to Game
	// Try a wide range since resolution/scaling can affect button positions
	exitButtonYPositions := []float64{0.45, 0.47, 0.49, 0.50, 0.52, 0.54, 0.56}
	...
	for attempt := 0; attempt < maxAttempts; attempt++ {
		...
		// First, try pressing ESC to open the quit menu
		fmt.Printf("Attempt %d: Pressing ESC to open quit menu...\n", attempt+1)
		gm.hid.PressKey(win.VK_ESCAPE)
		utils.Sleep(300) // Wait for menu to appear

		if !gm.gr.InGame() {
			return nil
		}

		// Check if the Quit Menu is open
		data := gm.gr.GetData()
		if data.OpenMenus.QuitMenu {
			fmt.Printf("Quit menu detected (attempt %d)\n", attempt+1)

			// Method 1: Keyboard navigation - Down, Down, Up, Enter
			// This ensures we always land on "Save and Exit Game" regardless of initial selection
			// Menu order: Options, Save and Exit Game, Return to Game
			// Down, Down puts us at "Return to Game", Up brings us to "Save and Exit Game"
			fmt.Println("Trying keyboard navigation: Down, Down, Up, Enter")
			gm.hid.PressKey(win.VK_DOWN)
			utils.Sleep(80)
			gm.hid.PressKey(win.VK_DOWN)
			utils.Sleep(80)
			gm.hid.PressKey(win.VK_UP)
			utils.Sleep(80)
			...
			// Method 2: If keyboard didn't work, try clicking at different Y positions
			yPos := exitButtonYPositions[attempt%len(exitButtonYPositions)]
			clickY := int(float64(gm.gr.GameAreaSizeY) * yPos)
			clickX := gm.gr.GameAreaSizeX / 2

			fmt.Printf("Keyboard didn't work, clicking at (%d, %d) - %.0f%% of screen height\n", clickX, clickY, yPos*100)
			gm.hid.Click(LeftButton, clickX, clickY)
			utils.Sleep(300)
			...
			// Menu didn't open - might have closed another panel, wait and retry
			fmt.Println("Quit menu not detected after ESC")
			utils.Sleep(200)
		}
	}
