...
const paladinRedemptionRaisersRange = core.ScreenRange + core.MeleeRange // We consider the screen range + a little buffer in case we have to move
const paladinRedemptionReplenishRange = 10                               // Game says 10.6 yards

//endregion Constants

//region Types

// PaladinOptions groups paladin-specific behavior knobs.
type PaladinOptions struct {
	...
	RedemptionHpThreshold    int // Percent (1-100) to trigger Redemption on low life.
	RedemptionManaThreshold  int // Percent (1-100) to trigger Redemption on low mana.
}
...
// auraOptions defines allowed paladin aura names.
...
func paladinDefaultOptions() PaladinOptions {
	return PaladinOptions{
		...
		RedemptionHpThreshold:    50,
		RedemptionManaThreshold:  50,
	}
}
...
func paladinOptionsForBuild(cfg *config.CharacterCfg, build string, isLevelingRun bool) PaladinOptions {
	...
	// Force defaults on leveling since we drive them from leveling module
...
// TODO: A bit hacky but the core does not really allow for such behavior atm
func (p *PaladinBase) ensureHolyShield() {
	if p.Data.PlayerUnit.States.HasState(state.Holyshield) {
		return
	}
	if !p.CanUseSkill(skill.HolyShield) {
		return
	}
	mana, _ := p.Data.PlayerUnit.FindStat(stat.Mana, 0)
	if mana.Value < 35 {
		return
	}
	if kb, kbFound := p.Data.KeyBindings.KeyBindingForSkill(skill.HolyShield); kbFound {
		p.HID.PressKeyBinding(kb)
		time.Sleep(180 * time.Millisecond)
		p.HID.Click(game.RightButton, 640, 340)
	}
}

//endregion Aura Resolution
