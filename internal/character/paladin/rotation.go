...
func (p *PaladinBase) shouldIgnoreMonsterDefault(monster data.Monster) bool {
	...
	case paladinMainFoh:
		packPlayer := p.scanSurroundingEnemiesDefault(p.Data.PlayerUnit.Position, core.ScreenRange)
...
func (p *PaladinBase) executeFohRotation(monster data.Monster) bool {
	packTarget := p.scanSurroundingEnemiesDefault(monster.Position, core.ScreenRange)
	packPlayer := p.scanSurroundingEnemiesDefault(p.Data.PlayerUnit.Position, core.ScreenRange)
