...
func (p *PaladinLeveling) updateOptionsForLeveling() {
	...
	enabledRunewordRecipes := []string{"Spirit", "Insight", "Cure", "Smoke", "Treachery"}
	// Runewords crafted during leveling.
	...
	if p.CharacterCfg.Game.Difficulty == difficulty.Normal {
		...
		if !p.Data.PlayerHasRuneword(item.RunewordStealth) {
			enabledRunewordRecipes = append(enabledRunewordRecipes, "Stealth")
		}
		if !p.Data.PlayerHasRuneword(item.RunewordAncientsPledge) {
			enabledRunewordRecipes = append(enabledRunewordRecipes, "Ancients' Pledge")
		}
		if !p.Data.MercHasRuneword(item.RunewordStrength) {
			enabledRunewordRecipes = append(enabledRunewordRecipes, "Strength")
		}
	}
	if !p.Data.PlayerHasRuneword(item.RunewordLore) {
		enabledRunewordRecipes = append(enabledRunewordRecipes, "Lore")
	}
	...
	if (p.Data.Quests[quest.Act2RadamentsLair].Completed() && !p.Data.Quests[quest.Act2TheSummoner].Completed()) ||
		(p.Data.Quests[quest.Act4TerrorsEnd].Completed() && !p.Data.Quests[quest.Act5RiteOfPassage].Completed()) {
		p.CharacterCfg.MaxGameLength = 1800 // Increase to 30mins for some parts of Act 2 and Act 5
	}
	...
	p.CharacterCfg.Game.Andariel.UseAntidoes = !p.Data.Quests[quest.Act1SistersToTheSlaughter].Completed()
